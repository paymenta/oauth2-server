...
PasswordGrantType.prototype.handle = function(request, client, req) {
  if (!request) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.getUser(request, req);
    })
    .then(function(user) {
...
PasswordGrantType.prototype.getUser = function(request, req) {
  ...
  return promisify(this.model.getUser, 3).call(this.model, request.body.username, request.body.password, req)
    .then(function(user) {
      if (!user) {
...
PasswordGrantType.prototype.saveToken = function(user, client, scope, req) {
  var fns = [
