...
AuthorizationCodeGrantType.prototype.handle = function(request, client, req) {
  if (!request) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.getAuthorizationCode(request, client, req);
    })
    .tap(function(code) {
      return this.validateRedirectUri(request, code);
    })
    .tap(function(code) {
      return this.revokeAuthorizationCode(code, req);
    })
    .then(function(code) {
      return this.saveToken(code.user, client, code.authorizationCode, code.scope, req);
    });
};
...
AuthorizationCodeGrantType.prototype.getAuthorizationCode = function(request, client, req) {
  ...
  return promisify(this.model.getAuthorizationCode, 2).call(this.model, request.body.code, req)
    .then(function(code) {
      if (!code) {
...
AuthorizationCodeGrantType.prototype.revokeAuthorizationCode = function(code, req) {
  return promisify(this.model.revokeAuthorizationCode, 2).call(this.model, code, req)
...
AuthorizationCodeGrantType.prototype.saveToken = function(user, client, authorizationCode, scope, req) {
  var fns = [
  ...
  return Promise.all(fns)
    ...
    .spread(function(scope, accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt) {
      ...
      return promisify(this.model.saveToken, 4).call(this.model, token, client, user, req);
    });
};
