...
ClientCredentialsGrantType.prototype.handle = function(request, client, req) {
  if (!request) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.getUserFromClient(client, req);
    })
    .then(function(user) {
      return this.saveToken(user, client, scope, req);
    });
};
...
ClientCredentialsGrantType.prototype.getUserFromClient = function(client, req) {
  return promisify(this.model.getUserFromClient, 2).call(this.model, client, req)
    .then(function(user) {
      if (!user) {
...
ClientCredentialsGrantType.prototype.saveToken = function(user, client, scope, req) {
  var fns = [
