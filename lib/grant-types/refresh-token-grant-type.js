...
RefreshTokenGrantType.prototype.handle = function(request, client, req) {
  if (!request) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.getRefreshToken(request, client, req);
    })
    .tap(function(token) {
      return this.revokeToken(token, req);
    })
    .then(function(token) {
      return this.saveToken(token.user, client, token.scope, req);
    });
};
...
RefreshTokenGrantType.prototype.getRefreshToken = function(request, client, req) {
  ...
  return promisify(this.model.getRefreshToken, 2).call(this.model, request.body.refresh_token, req)
    .then(function(token) {
      if (!token) {
...
RefreshTokenGrantType.prototype.revokeToken = function(token, req) {
  if (this.alwaysIssueNewRefreshToken === false) {
    return Promise.resolve(token);
  }

  return promisify(this.model.revokeToken, 2).call(this.model, token, req)
...
RefreshTokenGrantType.prototype.saveToken = function(user, client, scope, req) {
  var fns = [
