...
AuthenticateHandler.prototype.handle = function(request, response, req) {
  ...
  return Promise.bind(this)
    ...
    .then(function(token) {
      return this.getAccessToken(token, req);
    })
    .tap(function(token) {
    ...
    .tap(function(token) {
      ...
      return this.verifyScope(token, req);
    })
    .tap(function(token) {
...
AuthenticateHandler.prototype.getAccessToken = function(token, req) {
  return promisify(this.model.getAccessToken, 2).call(this.model, token, req)
...
AuthenticateHandler.prototype.verifyScope = function(accessToken, req) {
  return promisify(this.model.verifyScope, 3).call(this.model, accessToken, this.scope, req)
    .then(function(scope) {
      if (!scope) {
