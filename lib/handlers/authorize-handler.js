...
AuthorizeHandler.prototype.handle = function(request, response, req) {
  ...
  var fns = [
    ...
    this.getClient(request, req),
    this.getUser(request, response, req)
  ];
    ...
    .spread(function(expiresAt, client, user) {
      ...
      return Promise.bind(this)
        .then(function() {
          ...
          return this.validateScope(user, client, requestedScope, req);
        })
        .then(function(validScope) {
          scope = validScope;

          return this.generateAuthorizationCode(client, user, scope, req);
        })
        .then(function(authorizationCode) {
          state = this.getState(request);
          ResponseType = this.getResponseType(request);

          return this.saveAuthorizationCode(authorizationCode, expiresAt, scope, client, uri, user, req);
        })
        .then(function(code) {
...
AuthorizeHandler.prototype.generateAuthorizationCode = function(client, user, scope) {
  if (this.model.generateAuthorizationCode) {
    return promisify(this.model.generateAuthorizationCode, 4).call(this.model, client, user, scope, req);
  }
...
AuthorizeHandler.prototype.getClient = function(request, req) {
  ...
  return promisify(this.model.getClient, 3).call(this.model, clientId, null, req)
...
AuthorizeHandler.prototype.validateScope = function(user, client, scope) {
  if (this.model.validateScope) {
    return promisify(this.model.validateScope, 4).call(this.model, user, client, scope, req)
      .then(function (scope) {
        if (!scope) {
...
AuthorizeHandler.prototype.getUser = function(request, response, req) {
  if (this.authenticateHandler instanceof AuthenticateHandler) {
    return this.authenticateHandler.handle(request, response, req).get('user');
  }
  return promisify(this.authenticateHandler.handle, 3)(request, response).then(function(user) {
    if (!user) {
...
AuthorizeHandler.prototype.saveAuthorizationCode = function(authorizationCode, expiresAt, scope, client, redirectUri, user) {
  ...
  return promisify(this.model.saveAuthorizationCode, 4).call(this.model, code, client, user, req);
};
